...
lines 프로퍼티 타입 변경에 맞추어서 line이 추가되는 로직을 아래 코드와 같이 변경하였습니다. 그래서 그림을 그림으로써 새로운 터치 좌표가 추가될 때마다 직전 터치 좌표와 현재 터치 좌표를 연결하는 Line이 추가되도록 변경하였습니다.
```swift
final class CanvasView: UIView {
    ...  
    /*
...
따라서 CanvasView의 선이 매끄럽게 이어지지 않는 문제는 `lines` 프로퍼티와 `touchesBegan`, `touchesMoved` 를 수정하여 좌표를 저장하는 방식을 변경하였고, `draw` 메서드에서 선이 그려지는 방식을 변경함으로써 해결할 수 있었습니다.
